...
# AI / FLOOD RISK MODEL
# =========================================================
# flood_model.pkl is optional.
# If it is present, FloodGuard AI uses the Random Forest model.
# If it is absent, the app uses the built-in prototype risk engine.
...
model = None
model_source = "Built-in prototype risk engine"
...
try:
    ...
    model_source = "Random Forest model"
except Exception:
    model = None
...
def calculate_flood_probability(
    ...
    """Built-in illustrative risk engine used when no .pkl model is available."""
    rainfall_score = min(max(rainfall / 100.0, 0), 1)
    soil_score = min(max(soil_moisture / 100.0, 0), 1)
    water_score = min(max(water_level / 20.0, 0), 1)

    # Lower elevation = greater flood vulnerability.
    elevation_score = 1 - min(max(elevation / 500.0, 0), 1)
...
def get_weather_data(region, forecast_hours=24):
    ...
    if valid_soil:
        soil_moisture = valid_soil[0] * 100
    else:
        soil_moisture = None
...
st.caption(f"AI engine: {model_source}")
...
try:
    ...
    rainfall, soil_moisture, hourly_rainfall = get_weather_data(
        region,
        forecast_hours
    )

    if soil_moisture is None:
        raise ValueError("Soil moisture unavailable")
    ...
    # Fallback hourly rainfall data
    hourly_rainfall = [
        rainfall / forecast_hours
        for _ in range(forecast_hours)
    ]

    weather_status = "Prototype fallback data"
...
# Prototype conversion because the current trained model
# expects a smaller water-level scale.
...
# Get AI prediction probabilities.
if model is not None and hasattr(model, "predict_proba"):
    try:
        ...
        if 0 in classes:
            no_flood_probability = float(
                probabilities[classes.index(0)]
            ) * 100
        else:
            no_flood_probability = 100 - flood_probability
    except Exception:
...
st.caption(
    "These four environmental factors are supplied to the "
    "Random Forest model to generate the prototype "
    "flood-risk estimate."
)
...
with st.expander(
    ...
    """
    **1. Rainfall**  
    Forecast precipitation provides information about incoming water.

    **2. Soil Moisture**  
    Wetter soil can reduce infiltration and increase surface runoff.

    **3. Water Level**  
    River/water-level telemetry provides information about current water conditions.

    **4. Elevation**  
    Elevation provides geographical context for the selected location.

    These inputs are passed to the **Random Forest Classifier**, which produces
    the flood and no-flood probabilities displayed above.
    """
    )

    st.warning(
        "Prototype note: the current Random Forest was trained using sample "
        "training data. The probability should therefore be treated as an "
        "illustrative prototype result, not a scientifically validated flood forecast."
    )
...
imd_warning = selected_data["warning"]
...
with left:
    ...
    center_lat, center_lon = coordinates[region]
...
with right:
    ...
    &nbsp;&nbsp;
    <b>{round(feature_importance["rainfall"] * 100)}%</b>

    <br><br>

    💧 Soil moisture
    &nbsp;&nbsp;
    <b>{round(feature_importance["soil_moisture"] * 100)}%</b>

    <br><br>

    🌊 Water level
    &nbsp;&nbsp;
    <b>{round(feature_importance["water_level"] * 100)}%</b>

    <br><br>

    🏔️ Elevation
    &nbsp;&nbsp;
    <b>{round(feature_importance["elevation"] * 100)}%</b>

    <br><br>

    🛰️ Land cover
    &nbsp;&nbsp;
    <b>Not used by current model</b>
...
with b:
    ...
    <div class="small-text">
    Illustrative comparison across monitored regions
    </div>
    ...
    for city, values in location_data.items():
        ...
        if model is not None and hasattr(model, "predict_proba"):
            try:
                city_probabilities = model.predict_proba(city_input)[0]
                city_classes = list(getattr(model, "classes_", [0, 1]))

                if 1 in city_classes:
                    city_probability = float(
                        city_probabilities[city_classes.index(1)]
                    ) * 100
                else:
                    city_probability = 0.0
            except Exception:
                city_probability = calculate_flood_probability(
                    values["rainfall"],
                    values["soil_moisture"],
                    city_input.iloc[0]["water_level"],
                    values["elevation"]
                )
        else:
            city_probability = calculate_flood_probability(
        ...
        risk_values_all.append(
            round(city_probability)
        )
    ...
    risk_chart = pd.DataFrame({
        ...
        "Flood Probability": risk_values_all
    })
...
with r1:
    ...
    <div class="small-text">
    Used to observe land surface and
...
with r2:
    ...
    <div class="small-text">
    Helps estimate how saturated the
    ground is before heavy rainfall.
    </div>
...
with r3:
    ...
    <div class="small-text">
    Identifies low-lying terrain that
    may be vulnerable to flooding.
    </div>
...
if flood_probability >= 60:
    ...
    st.error(
        f"🔴 {risk_level} flood risk detected "
        f"for {region} for the {forecast.lower()}."
    )
...
st.caption(
    "FloodGuard AI • Hackathon prototype • "
    "Predictions are illustrative and should be "
    "validated against official information before "
    "real-world use."
)
